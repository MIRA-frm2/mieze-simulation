...
from elements.helmholtz_spin_flipper import Helmholtz_Spin_Flipper
...
def b_function(vec):
    ...
    y = vec[1]
    z = vec[2]
    return Helmholtz_Spin_Flipper.hsf(x,y,z, current=I_hsf1, mid_pos=0.1) + polariser.B_field(x,y,z) + earth_field


def main():


    Bx = lambda x,y,z: Real_Coil.Bx(x, coil_mid_pos=0.05, rho=np.sqrt(z**2+y**2), l=0.1, N=100, I=I_real_coil, R=0.05) +Real_Coil.Bx(x, coil_mid_pos=0.05, rho=np.sqrt(z**2+y**2), l=0.1, N=100, I=I_real_coil, R=0.05)
    By = lambda x,y,z: polariser.pol(x) + Real_Coil.Brho(x, coil_mid_pos=0.05, rho=y, l=0.1, N=100, I=I_real_coil, R=0.05)
    Bz = lambda x,y,z: Real_Coil.Brho(x, coil_mid_pos=0.05, rho=z, l=0.1, N=100, I=I_real_coil, R=0.05)
    ...
    #B_extra = Real_Coil.Bz(z=absolute_y_position, coil_mid_pos=0.05, rho=0.01, l=0.1, N=100, I=I_real_coil, R=0.05)
    #
